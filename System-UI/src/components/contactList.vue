<template>
  <div>
    ...
    <el-table :data="userList" style="width: 30; background-color: cadetblue;">
      ...
      <el-table-column prop="customerunit" label="客户单位">
      </el-table-column>
      ...
      <el-dialog :title="title" :visible="isShowChange" width="30%" :show-close="false">
        <el-form :model="user" label-width="80px">
          ...
          <el-form-item label="客户单位">
            <input type="text" v-model="user.customerunit"/>
          </el-form-item>
    ...
    <div>
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="curPage"
                     :page-sizes="[3, 4, 5]" :page-size="user.row" layout="total, sizes, prev, pager, next, jumper"
                     :total="totalCount">
      </el-pagination>
...
export default {
  ...
  methods: {
    load: function () {
      ...
      $.ajax({
        url: "http://localhost:8080/user/findAllManagerUserByPage",
      ...
      if (username != null && username.length != 0) {
        if (mythis.title == "添加") {
          $.ajax({
            ...
            data: {
              "username": username,
              "usertel": mythis.user.usertel,
              "useremail": mythis.user.useremail,
              "sex": mythis.user.sex,
              "customerunit": mythis.user.customerunit,
              "managername": managername,
    ...
    update: function (user) {
      ...
      mythis.user.customerunit = "";
      $.ajax({
        url: "http://localhost:8080/user/selectById",
    ...
    selectByuserName: function () {
      ...
      if (like != null && like.length != 0) {
        $.ajax({
          url: "http://localhost:8080/user/selectByuserName",
